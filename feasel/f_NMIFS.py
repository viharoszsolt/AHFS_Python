...
class NormalizedMutualInfo(FeaselBase):
    @property
    ...
    def __init__(self, dataset: np.ndarray, target: np.ndarray, selected: set[int] = None, n_features: int = 1, beta: float = 1.0, verbose: int = 0):
        """
        Implements the Normalized Mutual Information Feature Selection found in
        ...
        :type n_features: int
        :param beta: Weight of feature redundancy. Default value is 1.0.
        :type beta: float
    ...
    def transform(self) -> tuple[set[int], list[dict[int, float]], float]:
        ...
        while len(self.selected) < self.n_features:
            ...
            entropy_norm = np.min(np.concatenate([self.measures["Ei"][indices[:, 0]].reshape(-1, 1), self.measures["Ei"][indices[:, 1]].reshape(-1, 1)], axis = -1), axis = 1)
            measure = self.measures["MIij"][indices[:, 0], indices[:, 1]] / entropy_norm
