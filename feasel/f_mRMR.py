...
class MinRedMaxRel(FeaselBase):
    ...
    def __init__(self, dataset: np.ndarray, target: np.ndarray, selected: set[int] = None, n_features: int = 1, method: str = 'standard', verbose: int = 0):
        """
        Implements the Minimum Redundancy Maximum Relevance found in
        "Minimum redundancy feature selection from microarray gene expression data" by Ding et al.
        ...
        :type n_features: int
        :param method: Method to use; 'standard' implements the version in the original article, 'modified' changes minimum redundancy and overall score calculation. Default value is 'standard'.
        :type method: str
        ...
        assert self.method in ['standard', 'modified'], "method can only be either 'standard' or 'modified'!"
    ...
    def transform(self) -> tuple[set[int], list[dict[int, float]], float]:
        ...
        while len(self.selected) < self.n_features:
            ...
            if len(self.selected) > 1:
                tmp_ind = np.triu_indices(len(self.selected), k=1)
                sel_sorted = np.sort(np.array(list(self.selected)))
                selected_indices = np.column_stack((sel_sorted[tmp_ind[0]], sel_sorted[tmp_ind[1]]))

                sel_red = self.measures["MIij"][selected_indices[:, 0], selected_indices[:, 1]]
            else:
                sel_red = None
            ...
            red_measure = self.measures["MIij"][indices[:, 0], indices[:, 1]]
            if sel_red is not None: red_measure = np.append(red_measure, sel_red)
            ...
            if self.method == 'standard':
                if sel_red is not None: redundancy = {candidates[i][0]: np.sum(red_measure[measure_same_candidate[i]])/pow(len(measure_same_candidate[i]) + len(sel_red), 2) for i in range(len(candidates))}
                else: redundancy = {candidates[i][0]: np.sum(red_measure[measure_same_candidate[i]])/pow(len(measure_same_candidate[i]), 2) for i in range(len(candidates))}
            elif self.method == 'modified':
                redundancy = {candidates[i][0]: np.sum(red_measure[measure_same_candidate[i]]) for i in range(len(candidates))}
            else:
                raise ValueError("Undefined method value!")

            rel_measure = np.mean([self.measures["MIit"][indices[:, 0]], self.measures["MIit"][indices[:, 1]]], axis=0)
            relevance = {candidates[i][0]: np.mean(rel_measure[measure_same_candidate[i]]) for i in range(len(candidates))}

            feature_score = {}
            if self.method == 'standard':
                feature_score = {i: relevance[i] - redundancy[i] for i in redundancy.keys()}
            elif self.method == 'modified':
                feature_score = {i: relevance[i] / redundancy[i] for i in redundancy.keys()}
            else:
                raise ValueError("Undefined method value!")
