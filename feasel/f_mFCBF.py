...
class ModifiedFastCorrFS(FeaselBase):
    @property
    ...
    def __init__(self, dataset: np.ndarray, target: np.ndarray, n_features: int|None = None, threshold: float = 0.01, verbose: int = 0):
        """
        Implements the modified Fast Correlation Based Feature Selection found in
        "Fast Correlation Based Filter (FCBF) with a different search strategy" by Senliol et al.
        ...
        :param n_features: Number of features to select. If None, returns with a filtered feature order similar to FCBF. Default value is None.
        :type n_features: int
        ...
        if n_features is None:
            super().__init__(dataset, target, set(), 0, verbose)
        else:
    ...
    def transform(self) -> tuple[set[int], np.ndarray[int], float]:
        ...
        while i < len(SUit_sorted) - 1 and len(SUit_sorted) > self.n_features:
            ...
            try:
                to_remove = np.argwhere(self.measures["SUij"][indices[:, 0], indices[:, 1]] > self.measures["SUit"][SUit_sorted[i + 1:]])[-1] + (i + 1)
            except IndexError:
                to_remove = []

            if self.verbose > 1: print(f"{self._name}: Predominant feature is {SUit_sorted[i]}, removing feature(s) {SUit_sorted[to_remove]}..")

            if len(to_remove) > 0:
                SUit_sorted = np.delete(SUit_sorted, to_remove[0])
                none_removed = False
            i += 1

            if i >= len(SUit_sorted) - 1:
                if not none_removed:
                    i = 0
                    none_removed = True

        if SUit_sorted.shape[0] > self.n_features and self.n_features != 0:
