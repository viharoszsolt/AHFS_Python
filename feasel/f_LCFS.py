...
class LinearCorrelation(FeaselBase):
    @property
    ...
    def __init__(self, dataset: np.ndarray, target: np.ndarray, selected: set[int] = None, n_features: int = 1, beta: float = 0.5, verbose: int = 0):
        """
        Implements the Linear Correlation Feature Selection found in
        ...
        :type n_features: int
        :param beta: Weight of feature redundancy.
        :type beta: float
    ...
    def transform(self) -> tuple[set[int], list[dict[int, float]], float]:
        ...
        if len(self.selected) == 0:
            if self.verbose > 1: print(f"{self._name}: Selected features vector is empty, appending feature {np.argmax(self.measures['CORRit'])}..")

            self.selected.add(np.argmax(self.measures["CORRit"]))
            all_candidates.append({np.argmax(self.measures["CORRit"]): np.max(self.measures["CORRit"])})
        ...
        while len(self.selected) < self.n_features:
            ...
            measure = self.measures["CORRij"][indices[:, 0], indices[:, 1]]
            ...
            candidates_CORRij = {candidates[i][0]: np.sum(measure[measure_same_candidate[i]]) for i in range(len(candidates))}
            feature_score = {i: self.measures["CORRit"][i] - (self.beta / len(self.selected)) * candidates_CORRij[i] for i in candidates_CORRij.keys()}
